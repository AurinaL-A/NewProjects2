            ...
            <form class="p-4 md:p-5 space-y-4" method="POST" action="{{route('reports.store')}}">
                ...
                <div class="flex flex-col">
                    ...
                    <div>
                        ...
                        <x-text-input id="number" name="number" type="text"  class="block mt-1 py-2  " :placeholder="__('Регистрационный номер авто')" required />
                        <x-input-error :messages="$errors->get('number')" class="mt-2"/>
                    </div>
                    <!-- Desription -->
                    <div>
                        <x-input-label for="number" :value="__('Описание')"/>
                        <x-textarea id="description" class="block mt-1 py-2" rows="10" cols="35" name="description" :placeholder="__('Описание')"/>
                        <x-input-error :messages="$errors->get('number')" class="mt-2"/>
                    </div>
                    <!-- Status -->
